	...
	can.Control('Admin.Pages.Form', {}, {
		...
		render : function(nodes, nodeTypes, templateGroups){
			...
			this.element.html(this.view('init', {
				...
				languages      : Admin.Models.Languages.all(),
				hasSiteNode    : (nodes.length > 0)
			})).addClass('paper-form');
		...
		loadTemplates : function(){
			...
			if(this.type === ""){
				this.element.find('.parent-and-slug, .site-properties, .template-group-wrap').hide();
				return;
			}
			var isTypeSite = (this.type === "site");
			this.templateGroup = this.element.find('#page_template_group').val();
			this.element.find('.parent-and-slug').toggle(!isTypeSite);
			this.element.find('.site-properties').toggle(isTypeSite);
			if(this.type === "link"){
		...
		"{can.route} settings set" : "loadContentTab",
		"{can.route} behavior set" : "loadContentTab",
		loadContentTab : function(){
			...
			this._loadTimeout = setTimeout(this.proxy(function(){
				...
				var settingsAction = "loadSettingsFor" + can.route.attr('settings').camelize();
				this[settingsAction] && this[settingsAction]();
			}), 1)
			
		},
		loadSettingsForBehaviors : function(){
			...
			this.element.find('.tab-content').html(this.view('behavior', {
				model: this.options.page.attr(attrName),
				contentTypeDefinition : contentTypeDefinition,
				behavior : can.route.attr('behavior').humanize()
			}))
		},
		loadSettingsForSlots : function(){
			if(this.options.page.node_type === "link") return;
			$.when(
				Admin.Models.TemplateSlot.byFolderAndType(this.templateGroup, this.type),
				Admin.Models.TemplateSlot.defaultSlots(),
				Admin.Models.Slot.findAll({})
			).then(this.proxy('renderSettingsForSlots'))
		},
		renderSettingsForSlots : function(templateSlots, defaultSlots, existingSlots){
			templateSlots = templateSlots[0];
			existingSlots = existingSlots[0];
			templateSlots.setDefaultMappings(defaultSlots[0].slots || []);
			this.element.find('.tab-content').html(this.view('slots', {
				templateSlots: templateSlots,
				existingSlots: existingSlots,
				pageSlots    : (this.options.page.attr('slots') || {})
			}))
		},
